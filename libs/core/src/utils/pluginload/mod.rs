...
impl DownloadHandler {
    ...
    pub fn start(&self, link: &str, username: &str, password: &str) -> i32 {
        ...
        unsafe {
            let init_logger: Symbol<unsafe extern "C" fn(log_param: LogParam)> =
                self.lib.get(b"init_logger").unwrap();
            init_logger(log_param());
        }
        let symbol: Symbol<StartFunc> = unsafe { self.lib.get(b"start").unwrap() };
        unsafe { (symbol)(link.as_ptr(), username.as_ptr(), password.as_ptr()) }
    }
